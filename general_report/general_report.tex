...
\subsection{Machine Learning, Deep Learning y Modelos Estadísticos Clásicos}
    ...
    \subsubsection{SARIMAX}
        ...
        normal con media 0 y varianza constante. A través de un método iterativo de optimización numérica por Máxima Verosimilitud,
        se maximiza la función de Log-Verosimilitud:
        ...
        La versión definitiva del modelo convergió bajo un orden general uno, cero, cero y un orden estacional uno, cero, uno, siete.
        ...
        La sanidad matemática del sistema fue ratificada mediante el análisis del componente estocástico.
        A través de la prueba formal de Ljung-Box, se comprobó la hipótesis nula que dictamina que los residuos se distribuyen
        de manera independiente. Esto garantiza que el error remanente del modelo es simple ruido blanco y valida la
        integridad predictiva del diseño estadístico.

        Al someter el modelo entrenado sobre los datos reales del mes de abril, el comportamiento macroeconómico resultó sumamente preciso.
        La facturación real de la organización cerró en 34.29 millones de pesos, mientras que la proyección generada por el algoritmo anticipó
        un flujo de 35.20 millones de pesos. Este nivel de calibración se traduce en un desfase acumulado menor a un millón de pesos,
        equivalente a un margen de error total porcentual de apenas 2.64\% mensual y un error absoluto medio porcentual ponderado, onocido universalmente como WMAPE,
        de 27.30\% sobre la alta variabilidad diaria.
...
y_scaled = master['net_price'] / 1_000_000
X = master[['is_month_end', 'close_month_zone']]

y_scaled.index = pd.to_datetime(master['sale_date'])
...
order_final = (1, 0, 0)
seasonal_order_final = (1, 0, 1, 7)

model_final = SARIMAX(
    y_train_up_to_april,
    exog=X_train_up_to_april,
    order=order_final,
...
fit_res_final = model_final.fit(disp=False)

meta_final = save_metadata(
    order=list(order_final),
    seasonal_order=list(seasonal_order_final),
    fit_obj=fit_res_final,
    file_path=f'{MODELS_DIR}/sarimax_production.json'
)
    \end{lstlisting}
...
\end{figure}
    ...
    history = model.fit(
        ...
        \end{figure}
            ...
            Al disponer de un histórico de transacciones inferior a mil registros diarios, la red logró mapear de forma excelente
            ...
            pero careció de la densidad de ejemplos necesaria para dimensionar la agresividad financiera de los cierres de mes. 
...
se obtuvieron métricas de rendimiento que permiten contrastar la viabilidad operativa de cada enfoque matemático.
    ...
    \begin{itemize}
        ...
        27.3\% sobre la varianza diaria y un desfase macroscópico de apenas 2.64\% mensual. Su capacidad para asimilar las variables exógenas le
        ...
        \item Modelo LSTM.\\
        La red neuronal profunda exhibió un rendimiento intermedio en la escala diaria con un error del 30.64\%, pero falló significativamente en el
        acumulado mensual, subestimando la demanda real en un 14.41\%. Su comportamiento conservador derivó de la escasez relativa de datos históricos,
